...
def gen_overall_tab(results: dict, out_dir: str) -> None:
    ...
    for optimizer in results[problems[0]].keys():
        if optimizer == 'worst_cost' or optimizer == 'optimal_cost'or optimizer == 'num_instance':
            continue
    ...
    for problem in problems:
        df_results.loc['worst_cost',(problem,'mean')] = np.format_float_scientific(results[problem]['worst_cost'], precision=3, exp_digits=1)
        df_results.loc['worst_cost',(problem,'std')] = 0
        
    for problem in problems:
        df_results.loc['optimal_cost',(problem,'mean')] = np.format_float_scientific(results[problem]['optimal_cost'], precision=3, exp_digits=1)
        df_results.loc['optimal_cost',(problem,'std')] = 0
    
    for problem in problems:
        df_results.loc['num_instance',(problem,'mean')] = np.format_float_scientific(results[problem]['num_instance'], precision=3, exp_digits=1)
        df_results.loc['num_instance',(problem,'std')] = 0
