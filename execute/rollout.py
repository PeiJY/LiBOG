...
def rollout(opts,agent,epoch,tb_logger,tokenizer,testing=False):
    ...
    batch_size_each_task = {}
    for a in options.problem_id_each_task:
        if isinstance(a, list):  # 如果是列表
            length = len(a)
            for num in a:
                batch_size_each_task[num] = int(opts.ll_test_batch_size / length)
        else:
            batch_size_each_task[a] = opts.ll_test_batch_size
    ...
    with tqdm(range(len(test_id)),desc='rollout') as pbar:
        ...
        learning_env_list=[lambda e=None: L2E_env(dim=opts.dim,ps=opts.population_size,problem=e,max_x=opts.max_x,min_x=opts.min_x,max_fes=opts.max_fes,boarder_method=opts.boarder_method) for i in range(opts.rollout_batch_size)]
        ...
        if opts.teacher=='madde':
            ...
            teacher_env_list=[lambda e=None: MadDE(dim=opts.dim,problem=e,max_x=opts.max_x,min_x=opts.min_x,max_fes=opts.max_fes) for i in range(opts.rollout_batch_size)]
            ...
            teacher_env_list=[lambda e=None: sep_CMA_ES(dim=opts.dim,problem=e,max_x=opts.max_x,min_x=opts.min_x,max_fes=opts.max_fes,sigma=opts.cmaes_sigma) for i in range(opts.rollout_batch_size)]
            ...
            teacher_env_list=[lambda e=None: PSO(ps=opts.population_size,dim=opts.dim,max_fes=opts.max_fes,min_x=opts.min_x,max_x=opts.max_x,pho=0.2) for i in range(opts.rollout_batch_size)]
            ...
            teacher_env_list=[lambda e=None: DE(dim=opts.dim,ps=opts.population_size,min_x=opts.min_x,max_x=opts.max_x,max_fes=opts.max_fes) for i in range(opts.rollout_batch_size)]
        else:
        ...
        random_env_list=[lambda e=None: L2E_env(dim=opts.dim,ps=opts.population_size,problem=e,max_x=opts.max_x,min_x=opts.min_x,max_fes=opts.max_fes,boarder_method=opts.boarder_method) for i in range(opts.rollout_batch_size)]
        random_env=agent.vector_env(random_env_list)

        # task=TaskForTrain(learning_env,teacher_env,random_env,opts.batch_size,opts)
        task=TaskForTrain(learning_env,teacher_env,random_env,opts.rollout_batch_size,opts)
        
        for bat_id,id in enumerate(test_id):
            # generate batch instances for testing
            num_instance = batch_size_each_task[id]
            instances,p_name=sample_batch_task_id_cec21(dim=opts.dim,batch_size=num_instance,problem_id=id,seed=999)

                
            
            memory_imitate,bat_outperform_time,cost_stu,cost_base,cost_tea,worst_cost,optimal_cost=rollout_imitate(opts,task,agent,tokenizer,instances,testing)
            ...
            collect_dict[f'F{id}']['worst_cost']=worst_cost
            collect_dict[f'F{id}']['optimal_cost']=optimal_cost
            collect_dict[f'F{id}']['num_instance']=num_instance
            ...
            # save data
            path=os.path.join(opts.data_saving_dir,f'epoch_{epoch}_{opts.current_task_id}','test')
    ...
    test_outperform_ratio=test_outperform_time/(len(test_id)*opts.rollout_batch_size)
    # logging 
    if not opts.no_tb:
...
def rollout_imitate(opts,task,agent,tokenizer,instances,testing):
    ...
    worst_cost = np.max([p.gworst_cost for p in tea_pop])
    optimal_cost = [p.problem.optimum for p in tea_pop]
    # assert max(optimal_cost) == min(optimal_cost)
    if not max(optimal_cost) == min(optimal_cost):
        print(' !! optimal cost not identical, ',max(optimal_cost),min(optimal_cost))
    optimal_cost = min(optimal_cost)
    outperform_time=0
    is_done=False
    while not is_done:
    ...
    return memory,outperform_time,np.mean(memory.stu_cost[-1]),np.mean(memory.baseline_cost[-1]),np.mean(memory.teacher_cost[-1]),worst_cost,optimal_cost
