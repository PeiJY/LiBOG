...
def sample_batch_task_cec21_lifelong(config):
    problem_ids = options.problem_id_each_task[config.current_task_id]
    indicated_dataset = {key:value for key,value in cec2021.items() if value[0] in problem_ids}
        
    
    dataloader=Training_Dataset(filename=None, dim=config.dim, num_samples=config.batch_size, biased=False, shifted=True, rotated=True,
                            batch_size=config.batch_size,xmin=config.min_x,xmax=config.max_x,indicated_specific=True,indicated_dataset=indicated_dataset)
    ids=dataloader.keys
    problem_list=dataloader[0]
    return problem_list,ids
...
def get_train_set_cec21(config):
    ...
    # train_ids = options.problem_id_each_task[config.current_task_id]
    problem_list=[]
    dim=config.dim
    min_x=-100
    max_x=100
...
def get_train_set_cec21_lifelong(config):
    # train_ids=range(1,11)
    train_ids = options.problem_id_each_task[config.current_task_id]
    problem_list=[]
    dim=config.dim
    ...
    shifted=True
    rotated=True
    return_real_pro_name = []
    for id in train_ids:
        return_real_pro_name.append(real_pro_name[id-1])
    for id in train_ids:
        pro,subproblems,sublength,Comp_sigma,Comp_lamda,indicated_dataset=get_config(id)
        pro=Training_Dataset(filename=None, dim=dim, num_samples=1, problems=pro, biased=biased, shifted=shifted, rotated=rotated,
                            batch_size=1,xmin=min_x,xmax=max_x,indicated_specific=True,indicated_dataset=indicated_dataset,
                            problem_list=subproblems,problem_length=sublength,Comp_sigma=Comp_sigma,Comp_lamda=Comp_lamda)[0][0]
        problem_list.append(pro)
    return problem_list,return_real_pro_name
